...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['student_id'], $_POST['course_id'], $_POST['date'], $_POST['status'], $_POST['id'])) {
        $id = $_POST['id'];
        ...
        $update_sql = "UPDATE attendance4 SET student_id='$student_id', course_id='$course_id', date='$date', status='$status' WHERE id=$id";

        // Execute the update query
        if ($conn->query($update_sql) === TRUE) {
            echo "Attendance record updated successfully";
            // Redirect to the attendance records page
...
// Fetch the attendance record for editing
if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // SQL query to select the record based on the provided ID
    $select_sql = "SELECT * FROM attendance4 WHERE id = $id";
    $result = $conn->query($select_sql);

    // If the record is found, display the update form
