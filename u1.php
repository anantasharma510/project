...
if(isset($_GET['id'])) {
    ...
    // Fetch the attendance record based on the provided ID
    $select_sql = "SELECT * FROM attendance WHERE id = $id";
    $result = $conn->query($select_sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();

        // Handle form submissions for updating record
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            // Update query
            $update_sql = "UPDATE attendance SET student_id='$student_id', course_id='$course_id', date='$date', status='$status' WHERE id=$id";
...
<body>
    ...
    <!-- Form to update attendance record -->
    <form method="post">
